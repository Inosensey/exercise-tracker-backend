...
const exerciseSchema = new mongoose.Schema<ExerciseType>({
  ...
  name: {
    type: String,
    required: [true, "You must provide a name for the exercise"],
  },
  target: {
    type: String,
    required: [true, "You must specify the target muscle"],
  },
  difficulty: {
    type: String,
    required: [true, "You must provide a difficulty"],
  },
  exerciseSet: { type: String },
});
